...
dev_container: clean build
	sudo docker run -it $(DOCKER_RUN_FLAGS) -v $(WORK_DIR):/app/qdax $(DOCKER_IMAGE_NAME) /bin/bash
...
ipynb_container: clean build
	...
	sudo docker run -p 8888:8888 -it $(DOCKER_RUN_FLAGS) -v $(WORK_DIR):/app/qdax $(DOCKER_IMAGE_NAME) jupyter notebook --port=8888 --no-browser --ip=0.0.0.0 --allow-root

.PHONY: style
style: clean
	sudo docker run $(DOCKER_RUN_FLAGS) -v $(WORK_DIR):/app/qdax $(DOCKER_IMAGE_NAME) pre-commit run --all-files
