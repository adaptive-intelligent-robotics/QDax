...
def get_project_folder_name():
    return os.path.basename(os.path.dirname(os.getcwd())).strip().lower().replace(" ", "_")
...
def apply_changes(original_file: str,
    ...
    for line in original_file.splitlines():
        ...
        if "#CLONEHERE" in line:
            line = clone_commands(project_commit_ref_to_consider, ci_job_token, personal_token, project_name, no_check)
        fout.write(line + "\n")
    fout.close()
...
def generate_singularity_environment_variables(ci_job_token: Union[str, None],
    ...
    if project_folder \
            and key_singularityenv_project_folder not in os.environ:
...
def main():
    ...
    apply_changes(data, project_commit_ref_to_consider, ci_job_token, personal_token, project_name, no_check)

    # Create environment variables for singularity
    generate_singularity_environment_variables(ci_job_token, personal_token, project_folder=project_name)
