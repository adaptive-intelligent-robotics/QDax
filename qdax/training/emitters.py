...
def _apply_variation_iso_dd(iso_sigma, line_sigma, pparams_p1, pparams_p2, key):
    ...
    new_params = jax.tree_multimap(
        lambda x, y, a, b: x + a * iso_sigma + b * line_sigma * (x - y),
        pparams_p1,
        pparams_p2,
        noise_a,
        noise_b,
...
def _get_indexes(population_size, number_batches_to_select, repertoire, key):
    ...
    return [indiv_indices.at[idx].get() for idx in selected_indexes_list]
...
def get_select_from_repertoire_fn(population_size, number_batches_to_select):
    get_indexes_fn = functools.partial(
        _get_indexes, population_size, number_batches_to_select
    )
    select_from_repertoire_fn = functools.partial(
        _select_from_repertoire, get_indexes_fn
    )
...
def get_emitter_iso_line_dd(population_size: int, iso_sigma: float, line_sigma: float):
    return get_emitter_fn(
        selector_fn=get_select_from_repertoire_fn(
            population_size=population_size, number_batches_to_select=2
        ),
        variation_fn=get_apply_variation_iso_dd_fn(
