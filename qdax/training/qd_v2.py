...
def _eval_and_add(training_state: TrainingState,
  ...
  eval_scores, bds, dones, state = scoring_fn(pparams)
  ...
  # reward_trajectory = jnp.reshape(reward_trajectory, [1, -1, reward_trajectory.shape[-1]])
  ...
  # eval_scores = jnp.take_along_axis(reward_trajectory, first_done.reshape(1, len(first_done), 1), axis=2).ravel()
...
def _init_phase(get_random_parameters_fn,
                population_size,
                eval_and_add_fn,
                update_metrics_fn,
...
def train(
  ...
  logging.info("Batch size on 1 device for env: %d", num_envs // local_devices_to_use // process_count)

  # Initialize keys for random processes - need to handle for jax.
  key = jax.random.PRNGKey(seed)
  key, key_model, key_env = jax.random.split(key, 3) #key for main training state, policy model init and train environment

  timings.init_framework = time.time() -framework_t

  # Core training environment
  env_t = time.time() # NOTE: this timing doesnt work anymore at the moment - environment is initialized outside the train_fn
  ...
  repertoire = grid_archive.Repertoire.create(policy_params, min=min_bd, max=max_bd, grid_shape=grid_shape)
  ...
  eval_and_add_fn = jax.jit(functools.partial(
    ...
    add_to_archive_fn=jax.jit(repertoire.add_to_archive),
  ))
  ...
  training_state = TrainingState(
    ...
    metrics = Metrics.create(log_frequency = log_frequency, num_epochs = num_epochs, grid_shape = repertoire.grid_shape),
    state = first_state
  )
  # INIT REPERTOIRE #
  ...
  for i in range (1, num_epochs+1):
    ...
    timings.avg_eval_per_sec = ((i-1) * timings.avg_eval_per_sec + population_size/epoch_duration ) / float(i)
  ...
  inference = make_params_and_inference_fn(env.observation_size,
                                           env.action_size)
