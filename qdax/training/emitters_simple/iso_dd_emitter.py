...
def iso_dd_emitter(
    ...
    idx_p1 = jax.random.randint(
        key_select_p1, shape=(population_size,), minval=0, maxval=repertoire.num_indivs
    )
    idx_p2 = jax.random.randint(
        key_select_p2, shape=(population_size,), minval=0, maxval=repertoire.num_indivs
    )
...
def create_iso_dd_fn(
    population_size: int, iso_sigma: float = 0.01, line_sigma: float = 0.1
):
