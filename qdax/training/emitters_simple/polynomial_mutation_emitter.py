...
def polynomial_mutation_emitter(repertoire,
                                key, 
                                population_size: int,
                                eta_m: float):
    ...
    r = jax.tree_multimap(
        lambda g, k: jax.random.uniform(k, shape=g.shape, dtype=g.dtype), pparams,
        jax.tree_unflatten(treedef, all_keys_a))

    delta = jax.tree_map(lambda g: jnp.where(g<0.5, 
