...
class BraxTask(object):
    def __init__(
        ...
        self.step_fn = jax.jit(self.core_env.step)

        self.reset_fn = jax.jit(self.core_env.reset)

        self.reset_fn_batch = jax.jit(jax.vmap(self.core_env.reset))
        ...
        self.get_random_parameters = BraxTask._create_random_parameters_function(
            self.policy_model
        )
    ...
    def _create_get_bd_fn():
        return jax.jit(BraxTask._get_bd)

    @staticmethod
    def _create_eval_function(
        policy_model, reset_fn, step_fn, episode_length, action_repeat
    ):
    ...
    def _eval(
        ...
        (state, _, key, cumulative_reward, _), (
            obs,
            obs_weights,
            done,
            bd,
            ...
            do_one_step_fn,
            (state, params, key, cumulative_reward, active_episode),
            (),
    ...
    def _get_bd(obs_traj, first_done):
        ...
        bds = jnp.take_along_axis(
            bds, first_done.reshape(1, 1, len(first_done)), axis=1
        )
