...
class MLP(nn.Module):
    ...
    def __call__(self, x):
        for feat in self.features[:-1]:
            x = nn.relu(nn.Dense(feat, kernel_init=nn.initializers.variance_scaling(1.0, "fan_in", "uniform"))(x))
        return nn.Dense(self.features[-1], kernel_init=nn.initializers.variance_scaling(1.0, "fan_in", "uniform"))(x)
