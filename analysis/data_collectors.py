...
import operator
import os
from typing import Dict, Callable, Any, ValuesView, TypeVar
...
class DataCollectionAllVariants(object):
    def __init__(self,
        ...
        self.collection_from_variant_str_dict = {
            os.path.basename(variant_path): DataCollectionOneVariant(data_loader, variant_path)
            for variant_path in self.get_all_variants_folders(main_result_folder, experiment_name)
    ...
    def map_function_to_replication_collection(self,
                                               map_fn: Callable[['DataCollectionOneReplication'], TMapReturn],
    ...
    def apply_function_to_variant_scores(self, fn: Callable[[np.ndarray], np.ndarray], name_score):
        return {
    ...
    def get_common_attribute(self, get_attribute_fn):
        attribute_per_replication_per_variant = self.get_common_attribute_per_variant(get_attribute_fn=get_attribute_fn)

        if not DataCollectionAllVariants.all_equal(attribute_per_replication_per_variant.values()):
            raise AssertionError(f"The requested attribute is not unique among the different variants")
    ...
    def get_common_attribute_per_variant(self, get_attribute_fn):
        attribute_per_replication_per_variant = self.map_function_to_replication_collection(map_fn=get_attribute_fn)

        for attribute_per_replication in attribute_per_replication_per_variant.values():
            if not DataCollectionAllVariants.all_equal(attribute_per_replication.values()):
                raise AssertionError(f"The requested attribute is not unique among some replications")

        return self.map_and_reduce(
            map_fn=get_attribute_fn,
            reduce_fn=lambda x: next(iter(x)),  # getting first attribute (they are all equal anyway)
        )
...
class DataCollectionOneVariant(object):
    def __init__(self,
        ...
        self.collection_from_replication_str_dict = {
            os.path.basename(path_replication_one_variant): DataCollectionOneReplication(data_loader,
                                                                                         path_replication_one_variant)
            for path_replication_one_variant in self.get_all_paths_replications_one_variant(path_one_variant_folder)
    ...
    def get_all_paths_replications_one_variant(cls, path_one_variant_folder: str):
        ...
        for subdir_variant in all_subdirs_variant:
            ...
            if os.path.exists(os.path.join(path_subdir_variant, data_loaders.DataLoader.CONFIGURATION_FILE_NAME)):
    ...
    def apply_function_to_array_scores(self, fn, name_score: str):
        return fn(
            self.get_array_scores(name_score)
        )

    def map_function_to_replication_collection(self, map_fn: Callable[['DataCollectionOneReplication'], Any]):
        return {
...
def test():
    ...
    data_collection_all_variants = DataCollectionAllVariants(data_loader=data_loaders.DataLoaderPyMAPElites(),
    ...
    median_qd_scores = data_collection_all_variants.get_median_scores(name_score=data_loaders.DataLoader.QD_SCORE)

    first_quartile = data_collection_all_variants.get_quantile_scores(quantile=0.25,
                                                                      name_score=data_loaders.DataLoader.QD_SCORE)
    third_quartile = data_collection_all_variants.get_quantile_scores(quantile=0.75,
                                                                      name_score=data_loaders.DataLoader.QD_SCORE)

    # Plotting data_collection

    # epochs = data_collection_all_variants.get_median_scores(data_loaders.EPOCH)
    evaluations = data_collection_all_variants.get_median_scores(name_score=data_loaders.DataLoader.EVALUATIONS)

    for name_variant in median_qd_scores.keys():
        plt.plot(evaluations[name_variant],
                 median_qd_scores[name_variant]
                 )
        plt.fill_between(evaluations[name_variant], first_quartile[name_variant], third_quartile[name_variant],
                         alpha=0.3)
    ...
    data_collection_all_variants.save_serialised(name_file="data_collection_all_variants_pymapelites.pkl")

    # Getting all avg_eval_per_sec for example,
    avg_eval_per_sec_dict = data_collection_all_variants.map_function_to_replication_collection(
        map_fn=operator.attrgetter("timings.avg_eval_per_sec")
    )
    print("avg_eval_per_sec per replication", json.dumps(avg_eval_per_sec_dict, indent=4))
    ...
    print("median avg_eval_per_sec per variant", json.dumps(medians_avg_eval_per_sec_dict, indent=4))
    ...
    print("common attribute per variant (grid_shape)",
          data_collection_all_variants.get_common_attribute_per_variant(lambda x: x.configuration.grid_shape))
    print("common attribute (grid_shape)",
          data_collection_all_variants.get_common_attribute(lambda x: x.configuration.grid_shape))
