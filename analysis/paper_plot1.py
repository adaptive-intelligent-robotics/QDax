...
X_VALUE_NAME_TUPLE = ("Evaluations", "Epochs")
...
def generate_figure(path_save, attribute_comparison_name, data_collector_all_variants):
    ...
    fig_size = pu.get_fig_size(24, 6 * 3)
    ...
    for index_ax_row in range(0, len(X_VALUE_NAME_TUPLE)):
        x_value = X_VALUE_NAME_TUPLE[index_ax_row]

        for index_ax_column in range(0, len(METRICS_NAME_TUPLE)):
            # ax = axes.reshape(-1)[index_axis]
            ax = axes[index_ax_row, index_ax_column]

            name_metric = METRICS_NAME_TUPLE[index_ax_column]
            epochs_per_variant = data_collector_all_variants.get_median_scores(
                name_score=DataLoader.EPOCH
            )
            evaluations_per_variant = data_collector_all_variants.get_median_scores(
                name_score=DataLoader.EVALUATIONS
            )
            medians_per_variant = data_collector_all_variants.get_median_scores(
                name_score=name_metric
            )
            quartile_1_per_variant = data_collector_all_variants.get_quantile_scores(
                quantile=0.25, name_score=name_metric
            )
            quartile_3_per_variant = data_collector_all_variants.get_quantile_scores(
                quantile=0.75, name_score=name_metric
            )
            ...
            for index_variant, variant_folder in enumerate(
                ...
                x_quantity = {"Evaluations": num_evals, "Epochs": epochs}

                ax.plot(
                    x_quantity[x_value],  # num_evals,
                    medians,
            ...
            ax.set_title(METRICS_NAME_TUPLE[index_ax_column])

            ax.grid()
            ax.set_xlabel(x_value)

    for index_axis in range(0, len(METRICS_NAME_TUPLE)):
        ax = axes[2, index_axis]
        ...
        interpolate_scores_fn = get_interpolate_scores_fn(
            name_metric=name_metric
        )  # , fixed_interval_arr=fixed_interval_arr)
        ...
        medians_per_variant = data_collector_all_variants.map_and_reduce(
            map_fn=interpolate_scores_fn, reduce_fn=reduce_truncate_fn_median
        )
        quartile_1_per_variant = data_collector_all_variants.map_and_reduce(
            map_fn=interpolate_scores_fn, reduce_fn=reduce_truncate_fn_q1
        )
        quartile_3_per_variant = data_collector_all_variants.map_and_reduce(
            map_fn=interpolate_scores_fn, reduce_fn=reduce_truncate_fn_q3
        )
